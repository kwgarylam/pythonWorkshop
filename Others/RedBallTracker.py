...
def main():
    ...
    while cv2.waitKey(1) != 27 and capWebcam.isOpened():                # until the Esc key is pressed or webcam connection is lost
        ...
        #(180,255,255) to (0,0,0)
        imgThreshLow = cv2.inRange(imgHSV, (0, 155, 100), (20, 255, 255))
        imgThreshHigh = cv2.inRange(imgHSV, (165, 155, 100), (179, 255, 255))

        cv2.imshow("imgThresh_low", imgThreshLow) 
        cv2.imshow("imgThresh_High", imgThreshHigh) 


        imgThresh = cv2.add(imgThreshLow, imgThreshHigh)

        cv2.imshow("imgThresh_old", imgThresh) 
        ...
        circles = cv2.HoughCircles(imgThresh, cv2.HOUGH_GRADIENT, 2, intRows / 6)      # fill variable circles with all circles in the processed image
